...
Процедура РассчитатьСуммыПоКурсу()
	Если Не ЗначениеЗаполнено(ЭтотОбъект.Валюта) Или Не ЗначениеЗаполнено(ЭтотОбъект.Дата) Тогда
		Возврат;
	КонецЕсли;
	
	КурсВалюты = РегистрыСведений.КурсыВалют.Получить(
		Новый Структура("Валюта", ЭтотОбъект.Валюта),
		ЭтотОбъект.Дата);
