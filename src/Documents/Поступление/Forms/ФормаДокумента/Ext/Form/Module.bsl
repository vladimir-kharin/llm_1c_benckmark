...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектТовары = РеквизитФормыВОбъект("Объект");
	КоллекцияРеквизитов = ОбъектТовары.Метаданные().ТабличныеЧасти.Товары.Реквизиты;
	НовыйРеквизит = КоллекцияРеквизитов.Добавить();
	НовыйРеквизит.Имя = "СуммаВсегоРегл";
	НовыйРеквизит.Тип = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2));
	НовыйРеквизит.Синоним = "Сумма всего регл";
	
	// Добавление реквизита "ОбщаяСуммаВсегоРегл" на форму
	КоллекцияРеквизитовФормы = ЭтотОбъект.Реквизиты;
	НовыйРеквизитФормы = КоллекцияРеквизитовФормы.Добавить("ОбщаяСуммаВсегоРегл", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	
	// Вывод реквизита "СуммаВсегоРегл" в таблицу Товары
	Элементы.Товары.Добавить("ТоварыСуммаВсегоРегл", Тип("ПолеФормы"), Элементы.Товары);
	Элементы["ТоварыСуммаВсегоРегл"].Вид = ВидПоляФормы.ПолеВвода;
	Элементы["ТоварыСуммаВсегоРегл"].ПутьКДанным = "Объект.Товары.СуммаВсегоРегл";
	Элементы["ТоварыСуммаВсегоРегл"].ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
	
	// Размещение "ОбщаяСуммаВсегоРегл" в группе "ГруппаИтоги"
	Элементы.ГруппаИтоги.Добавить("ОбщаяСуммаВсегоРегл", Тип("ПолеФормы"), Элементы.ГруппаИтоги);
	Элементы["ОбщаяСуммаВсегоРегл"].Вид = ВидПоляФормы.ПолеВвода;
	Элементы["ОбщаяСуммаВсегоРегл"].ПутьКДанным = "ОбщаяСуммаВсегоРегл";
	Элементы["ОбщаяСуммаВсегоРегл"].ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
КонецПроцедуры
