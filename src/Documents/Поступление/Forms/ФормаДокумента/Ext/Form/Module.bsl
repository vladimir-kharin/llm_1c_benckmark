...
Процедура ОбработкаВводаДопРасходов(ВведенноеЧисло, ДополнительныеПараметры) Экспорт
	...
	ПересчитатьСуммуВсегоРеглНаКлиенте();
	КонецЕсли;
КонецПроцедуры
...
Процедура ПересчитатьСуммуВсегоРегл()
	...
	ЭтотОбъект.ОбщаяСуммаВсегоРегл = 0;
	Для каждого СтрокаТовары Из Объект.Товары Цикл
		ЭтотОбъект.ОбщаяСуммаВсегоРегл = ЭтотОбъект.ОбщаяСуммаВсегоРегл + СтрокаТовары.СуммаВсегоРегл;
	КонецЦикла;
КонецПроцедуры
