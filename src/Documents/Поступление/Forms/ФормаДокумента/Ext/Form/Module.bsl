...
Процедура ДобавитьРеквизитыНаФорму()
	...
	РеквСуммаРегл = Товары.Реквизиты.Добавить("СуммаВсегоРегл", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	
	// Добавляем поле в таблицу Товары
	ЭлементТаблицы = Элементы.Товары.ДочерниеЭлементы.Добавить(Тип("ПолеФормы"));
	ЭлементТаблицы.Вид = ВидПоляФормы.ПолеВвода;
	ЭлементТаблицы.Имя = "ТоварыСуммаВсегоРегл";
	ЭлементТаблицы.Заголовок = "Сумма всего (регламент)";
	ЭлементТаблицы.ПутьКДанным = "Товары.СуммаВсегоРегл";
	ЭлементТаблицы.Формат = "ЧЦ=15; ЧДЦ=2; ЧНС=;"; // Числовой формат: 15 цифр, 2 знака после запятой, без символа валюты
	
	// Добавляем общий реквизит формы
	РеквОбщаяСумма = Реквизиты.Добавить("ОбщаяСуммаВсегоРегл", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	
	// Добавляем поле в группу итогов
	ЭлементИтоги = Элементы.Добавить(Тип("ПолеФормы"), "ОбщаяСуммаВсегоРегл");
	...
	ЭлементИтоги.Формат = "ЧЦ=15; ЧДЦ=2; ЧНС=;"; // Числовой формат: 15 цифр, 2 знака после запятой, без символа валюты
	ЭлементИтоги.Родитель = Элементы.ГруппаИтоги;
КонецПроцедуры
