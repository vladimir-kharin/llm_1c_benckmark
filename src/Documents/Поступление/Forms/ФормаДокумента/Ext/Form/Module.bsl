...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Рассчитываем суммы по курсу при создании
	РассчитатьСуммыПоКурсуНаСервере();
КонецПроцедуры

&НаСервере
Процедура РассчитатьСуммыПоКурсуНаСервере()
	Если Не ЗначениеЗаполнено(Объект.Валюта) Или Не ЗначениеЗаполнено(Объект.Дата) Тогда
		Возврат;
	КонецЕсли;
	
	КурсВалюты = ПолучитьКурсВалютыНаСервере(Объект.Валюта, Объект.Дата);
	...
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		СуммаВВалюте = СтрокаТовара.Сумма + СтрокаТовара.СуммаДопРасходы;
		СуммаВРублях = СуммаВВалюте * КурсВалюты.Курс / КурсВалюты.Кратность;
		СтрокаТовара.СуммаВсегоРегл = Окр(СуммаВРублях, 2);
	КонецЦикла;
	
	ОбщаяСумма = Объект.Товары.Итог("СуммаВсегоРегл");
	ЭтотОбъект.ОбщаяСуммаВсегоРегл = Окр(ОбщаяСумма, 2);
КонецПроцедуры
...
&НаКлиенте
Процедура РассчитатьСуммыПоКурсу()
	// Вызываем серверный расчет и обновляем интерфейс
	РассчитатьСуммыПоКурсуНаСервере();
	Элементы.Товары.Обновить();
...
Функция ПолучитьКурсВалютыНаСервере(Валюта, Дата)
	...
	Если Выборка.Следующий() Тогда
		Возврат Новый Структура("Курс, Кратность", Выборка.Курс, Выборка.Кратность);
	КонецЕсли;
