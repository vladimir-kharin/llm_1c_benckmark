...
Запрос = Новый Запрос("ВЫБРАТЬ РегистрСведений.КурсыВалют.СрезПоследних(Период = &Дата, Валюта = &Валюта) КАК Курсы");
Запрос.УстановитьПараметр("Дата", Объект.Дата);
Запрос.УстановитьПараметр("Валюта", Объект.Валюта);
Результат = Запрос.Выполнить().Взять();
ЗаписьКурсы = Результат.Получить("Курсы");
